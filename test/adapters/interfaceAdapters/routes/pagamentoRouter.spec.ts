...
describe("GET em /api/pagamentos/:id", () => {
  ...
  it("Deve retornar erro", async () => {
    PagamentoController.listaPagamento = jest.fn().mockResolvedValue(null);

    await supertest(server).get("/api/pagamentos/1").expect(404);
  });
});

describe("POST em /api/pagamentos/processamento/", () => {
  it("Deve atualizar um pagamento pelo id em caso de sucesso", async () => {
    registroPagamentoMock.statusPagamento =
  ...
  //   await supertest(server)
  //     .post(`/api/pagamentos/processamento/${pagamentoSucessoMock.pedidoId}`)
  //     .send(registroPagamentoMock)
  //     .set("Accept", "application/json")
  //     .expect(200)
  //     .then((response) => {

  //       expect(response.body.mensagem).toBe("Processamento já foi realizado");
  //     });
  // });
});
